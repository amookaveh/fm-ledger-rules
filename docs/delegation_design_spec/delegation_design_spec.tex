...
\title{Design Specification for Delegation and Incentives in Cardano \\
       {\small (Version 1.2, git revision \gitAbbrevHash\gitDirty)} \\
...
\item A stake key is associated with exactly zero or one stake pools at any
  ...
  in \cref{operational-key-certificates}\todo{Using operational keys is now
    compulsory, we need to update the document to reflect that.}, and there can
...
\item
  ...
  wallets. Private pools (\cref{individual-staking}) will likely use this
  option.
\end{itemize}
...
the protocol (public key hashes, cost, margin, and pledge), while the metadata
will be displayed to end users by their wallet. For specifics about the
metadata, see \cref{stakepool-metadata}.
...
The stake pool registration certificate can contain a URL, pointing to a JSON
...
\todo{We need a business decision on whether we do want to store the metadata
  off-chain. We also need to converge on the actual metadata format and degree
  of flexibility to post arbitrary data.}
